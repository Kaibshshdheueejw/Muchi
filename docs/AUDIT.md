...
| Android audio + notification/lock-screen controls | ✅ | Native `MuchiAudio` plugin shipped (foreground service: ExoPlayer + MediaSessionCompat → background playback + OS notification + lock-screen controls; web fallback = WebView `<audio>` + MusicControls) — see MIGRATION.md §7 |
